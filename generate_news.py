...
・[マクロ・ビジネス系] (GLOBAL_WORLD, GLOBAL_BUSINESS, JAPAN_BUSINESS)
  => グローバルとローカルの動きをクロスリファレンスし、社会・市場全体に影響があるものを最大30件（できる限り30件）抽出してください。
・[テクノロジー・開発者系] (HACKER_NEWS, TECHCRUNCH, ZENN)
  => 概要（本文抜粋）を深く読み込み、**「なぜそれが面白いのか」「日本の開発者にどういう影響があるか」を含めた具体的なトピックを最大30件（できる限り30件）抽出**して列挙してください。
